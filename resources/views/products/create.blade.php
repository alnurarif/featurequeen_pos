    ...
    <section class="content">
      <div class="row">
        ...
        <div class="col-md-12">
          ...
          <div class="form-group">
                    <label>Color</label>
                    <input name="color" type="text" class="form-control" placeholder="Color" value="{{old('color')}}">
                  </div>
                </div>
                    ...
                    <select name="brand_id" class="form-control select2" style="width: 100%;">
                      ...
                      @if(count($brands)>0)
                        @foreach($brands as $brand)
                          <option value="{{$brand->id}}" @if(old('is_active')==$brand->id) selected="selected" @endif>{{$brand->name}}</option>  
                        @endforeach
                      @endif
                  </select>
                  </div>
                    ...
                    <select name="category_id" class="form-control select2" style="width: 100%;">
                      ...
                      @if(count($categories)>0)
                        @foreach($categories as $category)
                          <option value="{{$category->id}}" @if(old('is_active')==$category->id) selected="selected" @endif>{{$category->name}}</option>  
                        @endforeach
                      @endif
                  </select>
                  </div>
